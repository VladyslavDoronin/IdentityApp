...
namespace Api.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ResendEmailConfirmationLink(string email)
        {
            ...
            if (user == null) return Unauthorized("This email address has not been registeres yet");
            if (user.EmailConfirmed) return BadRequest("Your email address was confirmed before. Please login to you account");
        ...
        public async Task<IActionResult> ForgotUserNameOrPassword(string email)
        {
            ...
            if (user == null) return Unauthorized("This email address has not been registeres yet");
            if (user.EmailConfirmed == false) return BadRequest("Please confirm your eamil address first");
        ...
        private async Task<bool> GoogleValidatedAsync(string accessToken, string userId)
        {
            ...
            DateTime expiratin = DateTimeOffset.FromUnixTimeSeconds((long)payload.ExpirationTimeSeconds).DateTime;
            if (now > expiratin)
            {
                return false;
            }
        ...
        private async Task SaveRefreshTokenAsync(User user)
        {
            ...
            var existingRefershToken = await _context.RefreshTokens.SingleOrDefaultAsync(x=>x.UserId== user.Id);
            if(existingRefershToken != null)
            {
                existingRefershToken.Token = refreshToken.Token;
                existingRefershToken.DateCreated = refreshToken.DateCreated;
                existingRefershToken.DateExpiresUtc = refreshToken.DateExpiresUtc;
            }
            else
            {
